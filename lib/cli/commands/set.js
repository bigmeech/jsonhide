...
const DEFAULT_PUBLIC_KEY = '.jsoncrypt';
function parseCommandFn(yargs) {
    yargs
        ...
        .options('public-key', {
            alias: 'p',
            describe: 'Path to public key (Looks for .jsoncrypt in project root if not provided)',
            type:'string',
        })
}
...
exports.handler = (options) => {
    const pathToDefaultPublicKey = path.resolve(__dirname, process.cwd(), '.jsoncrypt')
